...
LOG_DIR = WORKING_DIR + '/spidy/logs'
SAVED_DIR = WORKING_DIR + '/spidy/saved'

# Open log file for logging
try:
    makedirs(LOG_DIR)  # Attempts to make the logs directory
    makedirs(SAVED_DIR)  # Attempts to make the saved directory
except OSError:
    pass  # Assumes only OSError wil complain if /logs already exists

LOG_FILE = open(path.join(LOG_DIR, 'spidy_log_{0}.txt'.format(START_TIME)),
                'w+', encoding='utf-8', errors='ignore')
LOG_FILE_NAME = path.join('spidy', 'logs', 'spidy_log_{0}'.format(START_TIME))

# Error log location
ERR_LOG_FILE = path.join(LOG_DIR, 'spidy_error_log_{0}.txt'.format(START_TIME))
ERR_LOG_FILE_NAME = path.join('spidy', 'logs', 'spidy_error_log_{0}.txt'.format(START_TIME))
                    ...
                    if COUNTER.val > 0:
                        try:
                            ...
                            if ZIP_FILES:
                                ...
                                # Save to zip files
                                zip_saved_files(time_string, SAVED_DIR)
                        finally:
                            # Reset variables
                            COUNTER = Counter(0)
                            WORDS.clear()
...
def save_page(url, page):
    ...
    file_path = path.join(SAVED_DIR, '{0}'.format(cropped_url))
...
def init():
    ...
    if USE_CONFIG:
        ...
        write_log('INIT', 'How many pages should spidy retrieve? (Default: 100):',
                  status='INPUT')
        input_ = input()
        if not bool(input_):
            MAX_CRAWL_PAGES = 100
